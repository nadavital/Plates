...
secret_mappings="${GEMINI_SECRET_NAME}=${GEMINI_SECRET_NAME}:latest,TRAI_ADMIN_API_KEY=${ADMIN_SECRET_NAME}:latest"
if [[ -n "${DATABASE_URL_SECRET_NAME:-}" ]]; then
  secret_mappings="${secret_mappings},TRAI_DATABASE_URL=${DATABASE_URL_SECRET_NAME}:latest"
fi

env_mappings="HOST=0.0.0.0,TRAI_ENVIRONMENT=${TRAI_ENVIRONMENT},TRAI_DATABASE_DRIVER=postgres,TRAI_DATABASE_SSL_MODE=${DATABASE_SSL_MODE},GEMINI_MODEL=${GEMINI_MODEL},ALLOW_DEV_APPLE_BYPASS=false,APPLE_EXPECTED_AUDIENCES=${APPLE_EXPECTED_AUDIENCES},APP_STORE_EXPECTED_BUNDLE_IDS=${APP_STORE_EXPECTED_BUNDLE_IDS}"
