...
- `GET /v1/admin/user-inspect`, `GET /v1/admin/users`, `GET /v1/admin/usage-summary`, `POST /v1/admin/reconcile-subscription`, `POST /v1/admin/quota-adjustment`, and `POST /v1/admin/quota-reset` provide support tooling for drift debugging, manual repair, and beta support credits.
...
- `GET /v1/admin/users` lists and searches backend users by email/display name/user ID, with effective subscription state and trailing 30-day usage. Use `node Backend/scripts/list_admin_users.mjs --query tester` with `TRAI_ADMIN_BASE_URL` and `TRAI_ADMIN_API_KEY` set for a compact support table.
